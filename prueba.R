...
author: "Lucía Vítores López"
---
  
library(dplyr) #verbos que resuleven desafios de manipulación mde datos más ocmunes (filter, mutate, select...)
library(magrittr)
library(tidyverse)
library(pdftools)
#library(stringr) viene incluido en tidyverse (permite trabajo fácil con cadenas)
library(lubridate) #para fechas y horas

#Determinamos la ruta donde se encuentra nuestro documento.
txt <- pdf_text("C://Users/luciv/OneDrive/Documentos/R/Sample_3_v88.pdf")
?pdf_text
...
#Separamos el texto de cada una de las líneas que lo forman
...
#ÚNICA TABLA CON TODA LA INFORMACIÓN
...
#Como vemos que los espacios en blanco también se almacenan, los quitamos
...
#Cambia los espacios de más de dos espacios (  ) por |
...
new_df <- raw_df %>% 
  separate(value, 
           into = c("NHC", "Fecha", "Chip", "% frecuencia alélica"), 
           sep = "\\|")
new_df
...
#DISTINTAS TABLAS CON DISTINTAS MENOS INFORMACIÓN
squished_table <- str_replace_all(table_trimmed, "\\s{2,}", "|") %>% 
  str_remove_all(",")
squished_table
