...
Importamos las bibiliotecas necesarias para desarollar el c?digo y garantizar su funcionamiento.

```{r}
library(dplyr) #verbos que resuleven desafios de manipulaci?n mde datos m?s comunes (filter, mutate, select...)
library(magrittr)
library(tidyverse)
library(pdftools)
...
Determinamos la ruta donde se encuentra nuestro docucmento. - Falta desarrollar la forma de solo ontroducir el documento y NO la ruta completa.
...
prueba_fecha
y<- dmy(prueba_fecha)
#Al rev?s porque mi ordenador est? configurado para poner primero a?o y no d?a
y
str(y)
...
```{r}
#Esta tabla contiene la relaci?n entre todas las mutaciones junto con el n?mero de mutaci?n que le corresponde. 
#(?la informaci?n nos la da Patricia o la busco en alguna base de datos determinada?)
