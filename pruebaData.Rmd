...
Nbiopsia
#Biopsia solida (depende del valor final del numero de biopsia: B=BIOPSIA, C=CITOLOGIA, P=PUNCION)
biopsia <- unlist(str_split(raw_text[7], "\\s{2,}"))[2]
biopsia
#Como la biopsia solida depende del ultimo caracter de Nbiopsia, uso este codigo para que me devuelva el ultimo valor
bsolida <- substr(as.character(biopsia), nchar(as.character(biopsia)), nchar(as.character(biopsia)))
bsolida
#Texto diagnostico
Texto <- raw_text[9]
Texto
...
mutacion
#Frecuencia alelica % (SIN PORCENTAJE)
raw_text[24]
frecuenciaAle <- unlist(str_split(raw_text[34], "\\s{2,}"))[5]
frecuenciaAle
frecuencia <- substr(frecuenciaAle, 1, 5)
frecuencia
#Numero mutacion
...
bm
#Numero diagnostico (le doy yo co la talba creada)
...
```{r}
#OTRA POSIBLE FORMA DE BUSCAR LA INFORMACIÓN SIN USAR LA POSICIÓN, BUSCANDO POR PALABRAS, SERÍA:
#Fecha informe
fechaI <- grep("Fecha:", raw_text)
fechaI
#grepl no me interesa, devuelve todo FALSE y TRUE cuando encuentra la palabra

f <-  unlist(str_split(fechaI, "\\s{2,}"))
f

biopsiaB <- grep("biopsia:", raw_text) 
biopsiaB

b <- unlist(str_split(biopsiaB, "\\s{2,}"))[2]
b

#Esto nos devuelve la posicion donde se encuentra la palabra, yo no quiero eso. 
#Una posible solucion seria buscar la palabra de interes para obtener su posición y una vez obtenida esta, hacer un bucle para que leyendo la posición, devuelva lo siguiente a la palabra que busco. MUCHO JALEO.
```


```{r}
#Uso de expresiones regulares
str_view(raw_text, "NHC") 

str_view(raw_text, "fase") 
str_view(raw_text, "Fecha:") 
str_view(raw_text, "biopsia:") 
str_view(raw_text, "Tipo de cáncer de la muestra:") 
str_view(raw_text, "Frecuencia de alelo:") 
str_view(raw_text, "IIC")
str_view(raw_text, "Ensayos clínicos")
str_view(raw_text, "Tratamiento relevante")
str_view(raw_text, "Gen")
```
